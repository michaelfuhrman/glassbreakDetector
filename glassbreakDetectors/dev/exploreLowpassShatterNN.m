...
function exploreLowpassShatterNN
close all
ramp_operator_setup;
Lpf=ramp.ops.lpf('fc',.5);
...
figure;
subplot(311)
sumChatter = (lpfShatter(idx2)-lpfShatter(idx1)); %/samples;
%plot(t(idx2),(lpfShatter(idx2)-lpfShatter(idx1))/4000,'.r',t,.1*l,'k');
plot(t(idx2),sumChatter,'.r','markersize',10);hold on;plot(t,l,'k');
title(['Difference of Lowpass Shatter NN Between 50msec to 200msec After Thud with Threshold Shown (' num2str(nnThresh) ')']);
