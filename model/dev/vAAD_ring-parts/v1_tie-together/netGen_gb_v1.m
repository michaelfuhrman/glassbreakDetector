...
%
function [net, features]=netGen_gb_v1(lib,ramp_ic,nn)
  ...
  features=[];
	...
	[atk, dec] = PDbias(0.7, 0.05, 3e3); atk = atk * 2; dec = dec / 3;
  ...
  % net.sub{end+1} = ramp_apeak_parasitic_local(lib, 'rms_4k', 'noise_4k', atkNoise, decNoise, buf, ch, 2);
	...
	[atk, dec] = PDbias(0.7, 0.05, 800); atk = atk * 3; dec = dec / 3;
	net.sub{end+1} = ramp_apeak_parasitic_local(lib, 'bpf_1k', 'rms_1k', atk, dec, buf, ch, 1);
  % net.sub{end+1} = ramp_alogshift(lib, 'rms_1k', 'rms_1k_log', gm, ioff, ch);
	% net.sub{end+1} = ramp_alpf_ms(lib, 'rms_1k', 'noise_1k', fbase, buf, ch, 6);
  % net.sub{end+1} = ramp_apeak_parasitic_local(lib, 'rms_1k', 'noise_1k', atkNoise, decNoise, buf, ch, 2);

	features{end+1} = struct('pos', 'rms_1k', 'neg', 'mid', 'ch', ch);

	ch = 3;
	net.sub{end+1} = ramp_abpf_Av(lib, 'in_gain', 'bpf_400', 4e2, Q, Av, buf, ch, 1);
	[atk, dec] = PDbias(0.7, 0.05, 350); atk = atk * 3; dec = dec / 3;
  ...
  % net.sub{end+1} = ramp_apeak_parasitic_local(lib, 'rms_400', 'noise_400', atkNoise, decNoise, buf, ch, 2);
  ...
  if isempty(nn)
    ...
    [~, net.sub{end+1}, ~] = nn_forward(nn, [], lib);
		...
		net.sub{end+1} = ramp_commonSource_pseudo(lib, 'thud_hang', 'thud_hang_inv', ch);
		...
		net.sub{end+1} = ramp_commonSource_pseudo(lib, 'shatter_hang', 'shatter_hang_inv', ch);
...
function [net]=vad_preroll(lib,in,out,vdd,...
  ...
  net.sub{end+1}=ramp_mscmp2(lib,pkpre,pktop,cmp_top,vth,vneg,cmp_gm,cmp_ibuf,chs(1));
  ...
  net.sub{end+1}=ramp_mscmp2(lib,pkbot,pkpre,cmp_bot,vth,vneg,cmp_gm,cmp_ibuf,chs(2));
...
function net = ramp_diffamp_pin(lib, inPos, inNeg, out, pin, chs)
  ...
  if ~isempty(lib)
		...
		net.hw{end+1} = lib.prim.resi('kilo', tap_pos,	'mega', amp_pos,	'loc',	[0 3]);
		net.hw{end+1} = lib.prim.resi('kilo', amp_pos,	'mega', mid_buff,	'loc',	[0 4]);
		net.hw{end+1} = lib.prim.opamp('pos', dc_ref, 'neg', mid_buff, 'out', mid_buff, 'bias', 50e-9, 'loc',[0 3]);

		% Negative side
		net.hw{end+1} = lib.prim.resi('kilo', inNeg,		'mega', tap_neg,	'loc',	[0 7]);
		net.hw{end+1} = lib.prim.resi('kilo', tap_neg,	'mega', amp_neg,	'loc',	[0 6]);
		net.hw{end+1} = lib.prim.resi('kilo', amp_neg,	'mega', out,			'loc',	[0 5]);
...
function net = ramp_commonSource_pseudo(lib, In, Out, chan)
  ...
  if ~isempty(lib)
    net.hw{end+1}=lib.prim.mifg('gate1', In, 'gate2', In, 'drain', Out, ...
																'sourceCur', 1e-6, 'drainCur', 0.3e-6, ...
																'pseudo', 1, 'loc', [5 chan]);
	end
end
