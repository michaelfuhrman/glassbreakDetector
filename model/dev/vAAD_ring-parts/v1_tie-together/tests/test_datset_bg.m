...
for i = 1:20 %length(t)
	for d = 1:length(dBspls)
		...
		Fs = 16e3;
		x_i = x(i); t_i = t(i); l_i = l(i);
		x_in = [zeros(8e3, 1); x_i(t_i<10)];
		l_in = [zeros(8e3, 1); l_i(t_i<10)];
...
% save test_dataset_bg_shatter_declare_id68_run0 dBspls metrics; % 40min after programming
% save test_dataset_bg_shatter_declare_id67_run0 dBspls metrics; % 40min after programming
% save test_dataset_bg_thud_declare_id68_run0 dBspls metrics; % A few hours after programming
