...
function [net, features]=netGen_gb(lib,ramp_ic,nn)
  ...
  net.in{end+1} = struct('net','inNeg', 'pin','A2');
	...
	% Diff amp
	net.sub{end+1} = ramp_diffamp_pin(lib, 'inPos', 'inNeg', 'diff_amp', 'A3', [2:5 1 6]);
  ...
  % net.sub{end+1} = ramp_apeak_parasitic_local(lib, 'rms_4k', 'noise_4k', atkNoise, decNoise, buf, ch, 2);
	...
	ch = 4;
	net.sub{end+1} = ramp_abpf_Av(lib, 'inPos', 'bpf_1k', 1e3, Q, Av, buf, ch, 1);
	[atk, dec] = PDbias(0.7, 0.05, 800); atk = atk * 2; dec = dec / 2;
  ...
  % net.sub{end+1} = ramp_apeak_parasitic_local(lib, 'rms_1k', 'noise_1k', atkNoise, decNoise, buf, ch, 2);
	...
	ch = 3;
	net.sub{end+1} = ramp_abpf_Av(lib, 'inPos', 'bpf_400', 4e2, Q, Av, buf, ch, 1);
	[atk, dec] = PDbias(0.7, 0.05, 350); atk = atk * 2; dec = dec / 2;
  ...
  if isempty(nn)
    ...
    [~, net.sub{end+1}, ~] = nn_forward(nn, [], lib);
		...
		preroll_gain = 5;
		ch = 0;
		net.sub{end+1} = ramp_again(lib, 'inPos', 'preroll_in', preroll_gain, ch);
		...
		pulse_down	= 7500;
		pulse_gm		= 1e-6;
		net.sub{end+1} = vad_preroll(lib, 'preroll_in', 'preroll_trig', 'vddLoc',...
...
function net = ramp_diffamp_pin(lib, inPos, inNeg, out, pin, chs)
  ...
  if ~isempty(lib)
		...
		% Positive side
		net.hw{end+1} = lib.prim.resi('kilo', inPos,		'mega', tap_pos,	'loc',	[0 1]);
		net.hw{end+1} = lib.prim.resi('kilo', tap_pos,	'mega', amp_pos,	'loc',	[0 2]);
		net.hw{end+1} = lib.prim.resi('kilo', amp_pos,	'mega', mid_buff,	'loc',	[0 3]);
		net.hw{end+1} = lib.prim.opamp('pos', dc_ref, 'neg', mid_buff, 'out', mid_buff, 'bias', 50e-9, 'loc',[0 3]);

		% Negative side
		net.hw{end+1} = lib.prim.resi('kilo', inNeg,		'mega', tap_neg,	'loc',	[0 6]);
		net.hw{end+1} = lib.prim.resi('kilo', tap_neg,	'mega', amp_neg,	'loc',	[0 5]);
		net.hw{end+1} = lib.prim.resi('kilo', amp_neg,	'mega', out,			'loc',	[0 4]);
