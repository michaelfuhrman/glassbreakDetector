function [thisChainName, scoreThud, scoreShatter] = glassBreakOptimizeParameters_v2(nnChainNumber,AtkDecParams,audioFileNumber)
...
switch nnChainNumber
    ...
    case 1
        Chain = getChainLog(ramp,AtkDecParams);
        sMethod='LogBaselineAndZCR';
    case 2
        % uses ramp.ideal.tanh instead of ramp.ideal.log
        Chain = getChainTanh(ramp,AtkDecParams);
        sMethod='Tanh';        
    case 3
...
gtZCR = ([ones(size(event_indexZCR)), zeros(size(noise_indexZCR))])';
gtThud = ([ones(size(event_indexThud)), zeros(size(noise_indexThud))])';

shatter = (ynnShatter([event_indexZCR, noise_indexZCR]));
thud = (ynnThud([event_indexThud, noise_indexThud]));

scoreShatter = sum((2*(gtZCR-.5).*(shatter-.5)))/length(gtZCR);
scoreThud = sum(((2*gtThud-.5).*(thud-.5)))/length(gtThud);
thisChainName = sMethod;

figure;
subplot(211)
plot(ynnThud([event_indexThud, noise_indexThud]))
title(['Thud Correlation = ' num2str(scoreThud)]);
hold on;
vline(length(event_indexThud),'r');
axis('tight')
ax(1)=gca;
subplot(212)
plot(ynnShatter([event_indexZCR, noise_indexZCR]))
title(['Shatter Correlation = ' num2str(scoreShatter)]);
hold on;
vline(length(event_indexZCR),'r');
ax(2)=gca;
axis('tight')
linkaxes(ax);
return
...
function Chain = getChainLog(ramp,atkdecparams)
atkdecparams= [8130,143,20,90,813,54,20,90];
%ramp_operator_setup;
gain=1;
...
function Chain = getChain3BandLog(ramp)
%ramp_operator_setup;
gain=1;
...
function Chain = getChainTanh(ramp,p)
%ramp_operator_setup;
gain=1;
...
Chain1=ramp.ops.bpf('fc',4e3, 'Av',gain) > ramp.ops.peak('atk',8.13e3, 'dec',143, 'modelVersion','PDmodelDynamic') > ramp.ideal.tanh('in_gain',20, 'out_min',0,'out_gain',.18) > ...
    [ramp.ideal.pass(); ...
    ramp.ops.peak('atk',p(1), 'dec',p(2), 'modelVersion','PDmodelDynamic')];
...
Chain2=ramp.ops.bpf('fc',4e2, 'Av',gain) > ramp.ops.peak('atk',8.13e2, 'dec',54, 'modelVersion','PDmodelDynamic') > ramp.ideal.tanh('in_gain',20, 'out_min',0,'out_gain',.18) > ...
    [ramp.ideal.pass(); ...
    ramp.ops.peak('atk',p(3), 'dec',p(4), 'modelVersion','PDmodelDynamic')];
...
function Chain = getChainZCREnvelope(ramp)
%ramp_operator_setup;
gain=1;
