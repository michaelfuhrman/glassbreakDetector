...
#
# Runs a single wav file through the pipeline of RAMPsim detection+preroll --> IAS glassbreak simulator
# Alternatively can run the file through just the IAS glassbreak simulator
#
# Arguments:
#   IN_WAV      = 1st argument, the input wav file
#   METHOD      = Current options are "FULL" for running both RAMPsim and IAS simulator
#                                     "IAS" for only running the IAS simulator
#
# Outputs:
#   Outputs to stdout a string telling the times of the RAMPsim detections and the times of the IAS detections
#   Format is:
...
# Binaries
# RAMPSIM="bin/RAMPsim/rampSim.exe"
RAMPSIM="bin/RAMPsim/rampSim.0.0.1.exe"
